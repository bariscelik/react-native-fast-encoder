import FastEncoder from './NativeFastEncoder';
...
export default class TextEncoder {
  ...
  decode(data: Uint8Array, props?: props) {
    if (!supportedEncodings[this.encoding]) {
      console.error(
        `unsupported encoding! encoding: ${this.encoding}, propd: ${props}, index: ${this.index}, value: ${JSON.stringify(data)}`
      );
    }
  ...
  private assureJSILoaded() {
    if (!this.loaded) {
      console.log(this.TAG, `(assureJSILoaded)`, 'JSI not loaded');
      this.loaded = FastEncoder.install();
      console.log(
        this.TAG,
        `(assureJSILoaded)`,
        'JSI install:',
